...
class MyDataloader(data.Dataset):
    ...
    def __init__(self, root, type, height, width, frame_idxs, num_scales,
                 sparsifier=None, modality='rgb', loader=None, samples=1000):
        ...
        self.crawl_folders()
        self.loader = loader
        self.sparsifier = uar(samples)  # TODO:采样点数量
        ...
        for scene in self.scenes:
            ...
            for i in range(-min(self.frame_idxs), len(imgs) - max(self.frame_idxs)):   # 掐头去尾
                ...
                for j in self.frame_idxs:
                    ...
                    if i + j != i:
                        t_vec = translation_list[i] - translation_list[i + j]
                ...
                sample = {'tgt': tgts, 'GT': gts, 'gt_rota': np.array(gt_rota), 'gt_tran': np.array(gt_tran),
    ...
    def __getitem__(self, index):
        ...
        # rgb = load_255(sample['tgt'][0])
        # depths = [load_float(depth) for depth in sample['GT']]
        ...
        for i in range(len(self.frame_idxs)):
            ...
            if self.transform is not None:
                rgb_np, gt_np = self.transform(rgb_np, gt_np)
            # else:
            #     raise (RuntimeError("transform not defined"))
            inputs[("color", self.frame_idxs[i], -1)] = Image.fromarray(rgb_np.astype(np.uint8))
        ...
        for scale in range(self.num_scales):
            ...
            inputs[("K", scale)] = torch.from_numpy(K).double()
            inputs[("inv_K", scale)] = torch.from_numpy(inv_K).double()
...
class aDataset(MyDataloader):
    ...
    def train_transform(self, rgb, depth):
        ...
        transform = transforms.Compose([
            transforms.Resize(370.0 / self.iheight),  # this is for computational efficiency, since rotation can be slow
        ...
        rgb_np = np.asfarray(rgb_np, dtype='float')
    ...
    def val_transform(self, rgb, depth):
        ...
        transform = transforms.Compose([
            transforms.Resize(370.0 / self.iheight),
            transforms.CenterCrop(self.output_size),
        ])
        rgb_np = transform(rgb)
        # rgb_np = np.asfarray(rgb_np, dtype='float') / 255
