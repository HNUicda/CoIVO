...
class Resize(object):
    ...
    def __init__(self, size, interpolation='nearest'):
        # TODO: 修改interpolation的预设值
        assert isinstance(size, int) or isinstance(size, float) or \
               (isinstance(size, collections.Iterable) and len(size) == 2)
        trans = {'nearest': Image.NEAREST}
        self.size = size
        self.interpolation = trans[interpolation]
    ...
    def __call__(self, img):
        ...
        """
        if img.ndim == 3:
            width = int(self.size * img.shape[0])
            height = int(self.size * img.shape[1])
            img = Image.fromarray(np.uint8(img))
            return np.asarray(img.resize((width, height), resample=self.interpolation))
            # return misc.imresize(img, self.size, self.interpolation)
        elif img.ndim == 2:
            width = int(self.size * img.shape[0])
            height = int(self.size * img.shape[1])
            o_shape = img.shape
            data32 = img.astype(np.float32)
            img = Image.frombytes('F', o_shape, data32.tostring())
            return np.asarray(img.resize((width, height), resample=self.interpolation))
