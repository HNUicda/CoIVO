...
class MyDataloader(data.Dataset):
    ...
    def __init__(self, root, type, height, width, frame_idxs, num_scales,
                 sparsifier=None, modality='rgb', loader=None):
        # 新初始化数据库（改自sequence_folders）
        self.root = Path(root)
        # print("_____"+self.root) # test
        self.height = height
        self.width = width
        ...
        self.crawl_folders()
        self.loader = loader
        self.sparsifier = uar(1000)  # TODO:采样点数量
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.frame_idxs)):
            ...
            # 生成m
            # map_np, n_mask = to_mmap(gt_np, sample['intrinsics'])  # 无效区域生成mask，255为无效，0为有效
            # rgbm_np = np.concatenate([rgb_np, map_np], axis=-1)
            # inputs[("rgbm", self.frame_idxs[i], -1)] = Image.fromarray(rgbm_np.astype(np.uint8))
            # lnm_np = np.concatenate([map_np, n_mask, np.zeros(map_np.shape)], axis=-1)
            # inputs[("lnm", self.frame_idxs[i], -1)] = Image.fromarray(lnm_np.astype(np.uint8))
...
class aDataset(MyDataloader):
    def __init__(self, root, type, height, width, frame_idxs, num_scales,
        ...
        super(aDataset, self).__init__(root, type, height, width, frame_idxs, num_scales,
                                       sparsifier=sparsifier, modality=modality, loader=loader)
